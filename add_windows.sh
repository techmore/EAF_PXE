...
echo "This script will install x86 / x64 versions of the following operating systems. 
 Edu buntu - A complete Linux based operating system targeted for primary and secondary education. It is freely available with community based support. The Edubuntu community is built on the ideas enshrined in the Edubuntu Manifesto: that software, especially for education, should be available free of charge and that software tools should be usable by people in their local language and despite any disabilities.
Kubuntu - An official derivative of Ubuntu Linux using KDE instead of the GNOME or Unity interfaces used by default in Ubuntu.
 ...
 Ubuntu GNOME - An official Ubuntu variant[56] that uses Gnome Shell as its default desktop and GDM as its display manager.
